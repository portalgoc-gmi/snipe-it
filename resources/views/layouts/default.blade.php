...
{{--                                        </li>--}}
                            ...
                            @if (Auth::check())
								...
								<li class="dropdown notifications-menu">
									<a href="#" class="dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
										...
										<span id="notif-badge" class="label label-danger" style="{{ $unreadCount > 0 ? '' : 'display:none' }}">
											{{ $unreadCount }}
										</span>
									...
									<ul class="dropdown-menu">
										<li class="header" id="notif-header">
											{{ $unreadCount > 0 ? "You have {$unreadCount} new notifications" : "No new notifications" }}
										</li>

										<li>
											<ul class="menu" id="notif-menu">
												<li><a href="#"><small>Loading...</small></a></li>
											</ul>
										</li>
                            ...
                            <li class="treeview{{ ((request()->is('statuslabels/*') || request()->is('hardware*')) ? ' active' : '') }}">
                                ...
                                <ul class="treeview-menu">
									...
									@php
									  ...
									  // 👑 Admin/Superadmin στο Snipe-IT (ΔΕΝ είναι group)
									  $isSuperAdmin = $me && ($me->isSuperUser() == 1);

									  // Groups που επιτρέπονται
									  $isWarehouseGroup = $me && $me->groups()
										  ->whereIn('name', ['Warehouse','Manager Archive','Archivist','Archivists'])
										  ->exists();

									  $canSeeReturns = $isSuperAdmin || $isWarehouseGroup;
									@endphp

									@php
									$returnsCounter = \App\Models\ReturnRequest::query()
									...
									@if($canSeeReturns)
									<li id="returns-sidenav-option" {!! (request()->is('returns*') ? ' class="active"' : '') !!}>
										<a href="{{ route('returns.index') }}">
											<x-icon type="circle" class="text-grey fa-fw"/>
											<strong>Returns</strong>

											@if($returnsCounter > 0)
												<span class="badge">{{ $returnsCounter }}</span>
											@endif
										</a>
									</li>
									@endif
        ...
        <script nonce="{{ csrf_token() }}">
			...
			function refreshNotificationCount() {
			  $.get("{{ route('notifications.dropdown') }}", function (res) {
				...
				if (count > 0) {
				  ...
				  $header.text('You have ' + count + ' new notifications');
				} else {
				  $badge.hide();
				...
				if (res.unread && res.unread.length) {
				  res.unread.forEach(function (n) {
					var msg = n.message ? ('<br><small>' + n.message + '</small>') : '';
					$menu.append(
					  '<li><a href="' + n.url + '">' +
				  ...
				  $menu.append('<li><a href="#"><small>No new notifications</small></a></li>');
				}
			  });
			}
			...
			$(document).ready(function () {
				...
				setInterval(refreshNotificationCount, 10000); // κάθε 10 sec

				$('.notifications-menu > a.dropdown-toggle').on('click', function () {
					refreshNotificationCount();
				});
			});
