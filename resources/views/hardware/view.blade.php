...
@php
    ...
    // είναι Secretary group;
    $isSecretary = $me && $me->groups()->where('name', 'Secretary')->exists();

    // Location του logged-in user (π.χ. Nuclear Department = 14)
    $myLocId = optional($me->location)->id;

    // Location του asset (το “Location” που βλέπεις στο asset page)
    $assetLocId = optional($asset->location)->id;

    // (προαιρετικό αλλά χρήσιμο) Location του user που είναι checked out to
    $assignedUserLocId = optional(optional($asset->assignedTo)->location)->id;

    // επιτρέπεται Return ΜΟΝΟ αν:
    // - είναι secretary
    // - το asset είναι checked out
    // - δεν είναι deleted
    // - και το location του secretary ταιριάζει με το location του asset
    //   (ή/και με το location του assigned user)
	...
	$openReturn = \App\Models\ReturnRequest::where('asset_id', $asset->id)
		...
		->whereNull('closed_at')   // ✅ open μέχρι να κλείσει
		->latest('requested_at')
		->first();
                        ...
                        <div class="info-stack-container">
                            ...
                            <div class="col-md-3 col-xs-12 col-sm-push-9 info-stack">
                                ...
                                @if ($asset->deleted_at=='')
                                    ...
                                    {{-- ✅ RETURNS (μόνο secretary του ίδιου LOCATION) --}}
									@if($canSecretaryReturn)
									  <div class="col-md-12 hidden-print" style="padding-top: 5px;">
										...
										@else
										  {{-- ✅ Αν η αποθήκη το παρέλαβε, δείξε Pending Check-in (όχι Return to Archive) --}}
										  @if($openReturn->received_at)
											<span class="label label-primary">Pending Check-in</span>
                                ...
                                {{-- labels --}}
                                <div class="col-md-12 hidden-print" style="padding-top: 5px;">
                                    <form method="POST" action="{{ route('hardware/bulkedit') }}" accept-charset="UTF-8" class="form-inline" target="_blank" id="bulkForm">
                                        @csrf
                                    ...
                                    <div class="col-md-12" style="text-align: left">
                                        ...
                                        <ul class="list-unstyled" style="line-height: 25px; font-size: 14px">
                                            ...
                                            @if ((isset($asset->assignedTo->employee_num)) && ($asset->assignedTo->employee_num!=''))
                                                <li><x-icon type="employee_num" class="fa-fw"/> {{ $asset->assignedTo->employee_num }}</li>
                                            @endif

                                            @if ((isset($asset->assignedTo->email)) && ($asset->assignedTo->email!=''))
                                                <li><x-icon type="email" class="fa-fw" /> <a href="mailto:{{ $asset->assignedTo->email }}">{{ $asset->assignedTo->email }}</a></li>
                                            @endif

                                            @if ((isset($asset->assignedTo)) && ($asset->assignedTo->phone!=''))
                                                <li><x-icon type="phone" class="fa-fw" /> <a href="tel:{{ $asset->assignedTo->phone }}">{{ $asset->assignedTo->phone }}</a></li>
                                            @endif

                                            @if((isset($asset->assignedTo)) && ($asset->assignedTo->department))
                                                <li><x-icon type="department" class="fa-fw" /> {{ $asset->assignedTo->department->name }}</li>
                                            @endif
                            ...
                            <div class="col-md-9 col-xs-12 col-sm-pull-3 info-stack">
                                <div class="row-new-striped">
                                    ...
                                    @if ($asset->last_checkout!='')
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>{{ trans('admin/hardware/table.checkout_date') }}</strong>
                                            </div>
                                            <div class="col-md-9">
                                    ...
                                    @if ((isset($audit_log)) && ($audit_log->created_at))
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>{{ trans('general.last_audit') }}</strong>
                                            </div>
                                            <div class="col-md-9">
                                    ...
                                    @if ($asset->next_audit_date)
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>{{ trans('general.next_audit_date') }}</strong>
                                            </div>
                                            <div class="col-md-9">
                                                {!! $asset->checkInvalidNextAuditDate() ? '<i class="fas fa-exclamation-triangle text-orange" aria-hidden="true"></i>' : '' !!}
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-9">
                                                <ul class="list-unstyled">
                                                    ...
                                                    @if ($asset->model->manufacturer->warranty_lookup_url!='')
                                                        <li>
                                                            ...
                                                            <a href="{{ $asset->present()->dynamicUrl($asset->model->manufacturer->warranty_lookup_url) }}" target="_blank">
                                                                ...
                                                                <x-icon type="external-link" />
                                                                <span class="sr-only">{{ trans('admin/hardware/general.mfg_warranty_lookup', ['manufacturer' => $asset->model->manufacturer->name]) }}</span>
                                                            </a>
                                                        </li>
                                                    @endif
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            <strong>{{ trans('admin/models/table.modelnumber') }}</strong>
                                        </div>
                                        <div class="col-md-9">
                                                ...
                                                <div class="col-md-9{{ (($field->format=='URL') && ($asset->{$field->db_column_name()}!='')) ? ' ellipsis': '' }}">
                                                    ...
                                                    @if ($field->isFieldDecryptable($asset->{$field->db_column_name()} ))
                                                        @can('assets.view.encrypted_custom_fields')
                                                            @php $fieldSize = strlen(Helper::gracefulDecrypt($field, $asset->{$field->db_column_name()})) @endphp
                                                            @if ($fieldSize > 0)
                                                                ...
                                                                @if (($field->format=='URL') && ($asset->{$field->db_column_name()}!=''))
                                                                    <span class="js-copy-{{ $field->id }} hidden-print" id="text-{{ $field->id }}-to-show" style="font-size: 0px;">
                                                                        <a href="{{ Helper::gracefulDecrypt($field, $asset->{$field->db_column_name()}) }}" target="_new">
                                    ...
                                    @if(($asset->components->count() > 0) && ($asset->purchase_cost))
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>{{ trans('admin/hardware/table.components_cost') }}</strong>
                                            </div>
                                            <div class="col-md-9">
                                                @if (($asset->id) && ($asset->location))
                                                    {{ $asset->location->currency }}
                                    ...
                                    @if ($asset->warranty_months)
                                        ...
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>{{ trans('admin/hardware/form.warranty_expires') }}</strong>
                                            </div>
                                            <div class="col-md-9">
                                                @if ($asset->purchase_date)
                                        ...
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>
                                                    ...
                                                    {!! ($asset->purchase_date && $asset->asset_eol_date < date("Y-m-d")) ? '<i class="fas fa-exclamation-triangle text-orange" aria-hidden="true"></i>' : '' !!}
                                                </strong>
                                            </div>
                                            <div class="col-md-9">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3">
                                            <strong>{{ trans('admin/hardware/form.notes') }}</strong>
                                        </div>
                                        <div class="col-md-9">
                                    ...
                                    @if ($asset->defaultLoc)
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>{{ trans('admin/hardware/form.default_location') }}</strong>
                                            </div>
                                            <div class="col-md-9">
                                                @can('superuser')
                                    ...
                                    @if ($asset->last_checkin!='')
                                        <div class="row">
                                            <div class="col-md-3">
                                                <strong>{{ trans('admin/hardware/table.last_checkin_date') }}</strong>
                                            </div>
                                            <div class="col-md-9">
                                ...
                                @foreach ($asset->licenseseats as $seat)
                                    @if ($seat->license)
                                        <tr>
                                            ...
                                            <td>
                                                @can('viewKeys', $seat->license)
                                                    <code class="single-line">
                                                        <span class="js-copy-link" data-clipboard-target=".js-copy-key-{{ $seat->id }}" aria-hidden="true" data-tooltip="true" data-placement="top" title="{{ trans('general.copy_to_clipboard') }}">
                                                            <span class="js-copy-key-{{ $seat->id }}">{{ $seat->license->serial }}</span>
                                                        </span>
                                                    </code>
                                                @else
                                                    ------------
                                            ...
                                            <td>{{ Helper::getFormattedDateObject($seat->license->expiration_date, 'date', false) }}</td>
                                            <td>
                                                <a href="{{ route('licenses.checkin', $seat->id) }}" class="btn btn-sm bg-purple hidden-print" data-tooltip="true">
                                    ...
                                    @foreach ($asset->components as $component)
                                        @if (is_null($component->deleted_at))
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('components.checkin.show', $component->pivot->id) }}" class="btn btn-sm bg-purple hidden-print" data-tooltip="true">
                                                        {{ trans('general.checkin') }}
                                                    </a>
                                                </td>
                        ...
                        <div class="col-md-12">
                            <table
                                ...
                                <tr>
                                    ...
                                    <th data-visible="false" data-field="file" data-formatter="fileDownloadButtonsFormatter">{{ trans('general.download') }}</th>
