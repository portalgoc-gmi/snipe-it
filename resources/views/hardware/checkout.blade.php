...
@section('moar_scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Το location id που μας έστειλε το request
            var requestedLocationId  = '{{ $requestedLocationId ?? '' }}';

            // Αν υπάρχει location από το request ➜ προεπιλογή στο dropdown
...
@section('moar_scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // --------- Προεπιλογή χρήστη από το τελευταίο request ----------
            ...
            if (requestedUserId) {
                ...
                if (userSelect) {
                    ...
                    // Αν το select είναι select2, trigger change για να φανεί σωστά
                    userSelect.dispatchEvent(new Event('change'));
                }
            }

            // (προαιρετικά, κράτα ή σβήσε το κομμάτι για το location)
            var requestedLocationId  = '{{ $requestedLocationId ?? '' }}';
            if (requestedLocationId) {
                var locationSelect = document.querySelector('select[name="assigned_location"]');
                if (locationSelect) {
