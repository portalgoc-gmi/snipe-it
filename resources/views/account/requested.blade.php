...
@php
    ...
    $isWarehouse = $user->groups()->whereIn('name', ['Admin', 'Warehouse', 'Archivist'])->exists();
...
@section('moar_scripts')
	...
	function requestedTransferFormatter(value, row) {
		...
		// Το request object έχει user (requesting user) και requestedItem (asset)
		const assetId = row.requested_item?.id || row.requestedItem?.id || row.asset_id || row.id;
		const requestedUserId = row.user_id || row.user?.id;  // αυτός που έκανε request

		if (!assetId || !requestedUserId) {
			return '-';
		}

		// Πάμε στο checkout form, ήδη το AssetCheckoutController@create κάνει prefill user
		const url =
