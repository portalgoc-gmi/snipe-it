      ...
      <div class="nav-tabs-custom">
        ...
        <div class="tab-content">
          ...
          <div class="tab-pane" id="assets">
            ...
            <table
			  data-cookie-id-table="userAssignedAssets_v2"
			  ...
			  <thead>
				<tr>
				  ...
				  <th class="col-md-2">Return Actions</th>
				</tr>
			  </thead>
			  ...
			  <tbody>
				...
				@foreach ($user->assets as $asset)
				  <tr>
					...
					{{-- Category --}}
					<td>
					  {{ optional(optional($asset->model)->category)->name }}
					</td>
...
@section('moar_scripts')
  ...
  <script>
    function postReturnAction(assetId, action) {
      return $.ajax({
        url: "{{ url('/returns') }}/" + assetId + "/" + action,
        type: "POST",
        headers: {
    ...
    $(document).on('click', '.js-mark-in-transit', function () {
      const assetId = $(this).data('asset-id');
      const $btn = $(this);
      $btn.prop('disabled', true);

      postReturnAction(assetId, 'mark-in-transit')
    ...
    $(document).on('click', '.js-return-to-archive', function () {
      const assetId = $(this).data('asset-id');
      const $btn = $(this);
      $btn.prop('disabled', true);

      postReturnAction(assetId, 'return-to-archive')
        .done(() => location.reload())
        .fail(xhr => {
          $btn.prop('disabled', false);
          alert('Error: ' + (xhr.responseJSON?.message || xhr.statusText));
  ...
  <script>
	  function refreshUserAssetsTbody() {
		// παίρνει ξανά το ίδιο page HTML και αντικαθιστά μόνο το tbody του #userAssets
	  ...
	  setInterval(function () {
		// μόνο αν είσαι στο Assets tab (προαιρετικό)
