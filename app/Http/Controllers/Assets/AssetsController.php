...
class AssetsController extends Controller
{
	...
	public function getRequestedIndex($user_id = null)
	{
		...
		$query = CheckoutRequest::with(['user', 'requestedItem'])
			...
			->where('requestable_type', \App\Models\Asset::class); // ✅ κράτα μόνο asset requests

		if (Schema::hasColumn('checkout_requests', 'fulfilled_at')) {
			$query->whereNull('fulfilled_at');
		}

		// ✅ μόνο το τελευταίο pending request για κάθε asset
		...
		// Ρόλοι που βλέπουν ΟΛΑ τα requests
		...
		/**
		 * Κανόνας για τμήματα:
		 * - Όλοι οι ΜΗ privileged βλέπουν ΜΟΝΟ requests για Assets που αυτή τη στιγμή
		 *   βρίσκονται στο ίδιο location (department) με τον χρήστη.
		 * - Έτσι το RAD δεν θα βλέπει requests που έκανε προς Warehouse κλπ.
		 */
		if (! $isPrivileged) {
			$locationId = $user->location_id;

			// Φιλτράρουμε ΜΟΝΟ requests που αφορούν Assets
			// και όπου το Asset.location_id == location του χρήστη
			$query->whereHasMorph(
				'requestedItem',
		...
		// (Προαιρετικό) φίλτρο ανά user_id, αν το χρειάζεσαι
		if (! empty($user_id)) {
