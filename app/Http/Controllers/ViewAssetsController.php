...
class ViewAssetsController extends Controller
{
    ...
    public function getRequestItem(Request $request, $itemType, $itemId = null, $cancel_by_admin = false, $requestingUser = null): RedirectResponse
    {
		...
		if (($item_request = $item->isRequestedBy($user)) || $cancel_by_admin) {
			...
			// 1) Βρες location με τον ίδιο τρόπο όπως στο request
			$location = null;
			...
			// 2) Στείλε notification σε ΟΛΟΥΣ τους users του location
			// (και προαιρετικά εξαιρείς αυτόν που το ακύρωσε)
			if ($location) {
				$recipients = \App\Models\User::where('activated', 1)
					->where('location_id', $location->id)
					->where('id', '!=', auth()->id()) // optional
					->get();

				\Illuminate\Support\Facades\Notification::send(
					$recipients,
					new \App\Notifications\RequestAssetCancelation($data)
				);
			}
			...
			if ($fullItemType === \App\Models\Asset::class) {
				...
				// rule for models/others (keep your rule)
				$location = auth()->user()->location;
			}

			// 2) Notify ALL users that belong to this location (not the location manager)
			if ($location) {
				$recipients = User::where('activated', 1)
					->where('location_id', $location->id)
					->where('id', '!=', auth()->id()) // optional: not the requester
					->get();

				Notification::send($recipients, new RequestAssetNotification($data));
				// ή αντί για Notification::send μπορείς:
				// foreach ($recipients as $u) { $u->notify(new RequestAssetNotification($data)); }
			}

			return redirect()
				->route('requestable-assets')
