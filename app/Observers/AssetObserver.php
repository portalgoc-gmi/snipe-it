...
class AssetObserver
{
	...
	{
		// ✅ ΜΟΝΟ στο create (όχι σε edit)
		if (!$asset->exists) {

			$me = Auth::user();

			// Groups που θεωρούμε "Warehouse/Archive"
			...
			// IDs από ονόματα
			$archiveLocationId   = Location::where('name', 'Archive')->value('id');
			$statusInArchiveId   = Statuslabel::where('name', 'In Archive')->value('id');
			$statusWithDeptId    = Statuslabel::where('name', 'With Department')->value('id');

			// ✅ Μην overwrite αν ο user διάλεξε ήδη Status/Location στο form
			$hasStatus   = !empty($asset->status_id);
			$hasLocation = !empty($asset->location_id);

			if ($isWarehouseUser) {
				// αποθηκάριος/archivist -> In Archive + Location Archive
				...
				// άλλος user (πχ secretary) -> With Department + Location = location του user
				...
				// ✅ Για να φαίνεται "Checked Out To" (όπως το 20003007):
				// κάν’το assigned στον creator, ΜΟΝΟ αν δεν έχει ήδη assigned
				if (empty($asset->assigned_to) && $me) {
					$asset->assigned_to   = $me->id;
					$asset->assigned_type = User::class; // important στο Snipe-IT
				}
			}
		}

		// --- το δικό σου EOL logic όπως ήταν ---
