...
class AssetPresenter extends Presenter
{
    ...
    public static function dataTableLayout()
    {
		...
		// Hide checkin/checkout column for Secretary group
		if (auth()->check() && auth()->user()->groups()->where('name', 'Secretary')->exists()) {
			$layout = array_values(array_filter($layout, function ($col) {
				return ($col['field'] ?? null) !== 'checkincheckout';
			}));
		}
		...
		if (auth()->check() && auth()->user()->groups()->where('name', 'Secretary')->exists()) {
			$layout = array_values(array_filter($layout, function ($col) {
				...
				return !in_array($field, ['checkincheckout','actions'], true);
			}));
		}
	...
	public static function dataTableLayoutDueCheckin(): string
    {
        $cols = json_decode(self::dataTableLayout(), true);
